...
const [alldata, setalldata] = useState([]);
  const handledata=async()=>{
    ...
    const res=await fetch('http://localhost:8080/api/allbuyerdata')
    const resdata=await res.json()
    console.log('get all adata supplier dataaaaaaaaaaaaaa',resdata.data)
setalldata(resdata.data)
      ...
      const handledelete=async(id)=>{
        ...
        const res=await fetch(`http://localhost:8080/api/deletebuyerdata/${id}`,{
          method:'delete',
        })
